...
class ActionValidateBooking(Action):
    ...
    WHY NOT IN A PROMPT?
    If you wrote "only confirm if deposit is under GBP 300" in a prompt,
    the LLM might reason: "the manager said it's a GBP 250 fee plus GBP 100
    insurance, so technically it's under GBP 300 each." Python doesn't negotiate.
    """
    ...
    def run(
       ...
       # -- TASK B: Cutoff Guard ---------------------------------------------
        ...
        if deposit > MAX_DEPOSIT_GBP:
            return escalate(
                f"a deposit of GBP{deposit:.0f} exceeds the organiser's "
                f"authorised limit of GBP{MAX_DEPOSIT_GBP}"
            )
        ...
        dispatcher.utter_message(
            text=(
                ...
                f"GBP{deposit:.0f} deposit accepted. "
